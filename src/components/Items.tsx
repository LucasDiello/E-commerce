...
function Items(items: Item[], detailedView: boolean) {
  ...
  return (
    items.map((item : Item) => {
      return <div key={item.id} className={` ${detailedView && 'shadow-items xs:w-[170px] p-2 mr-2 mb-2 w-[190px] h-[180px]'} ${!detailedView && 'carousel-item h-[130px]'}`}>
         <img src={item.thumbnail} alt={item.title} />
           <h3 className='text-[10px] mt-3 title-card'>{item.title.substring(0, detailedView ? 60 : 10)}</h3>
           {detailedView && (
             <div className='flex justify-between w-ful footer-card'>
